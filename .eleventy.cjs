...
module.exports = (eleventyConfig, options = {}) => {
    ...
    if (options.scriptAttributes) {
        hsScripts.scriptAttributes = JSON.stringify(options.scriptAttributes);
        delete options.scriptAttributes;
    }
    ...
    const buildScriptAttrsString = (attrsJson) => {
        ...
        (function(w,d,o,cb,observer,target) {
            ...
            if (entries[0].isIntersecting) {
                if ( !document.querySelector('[src="${hsScripts.forms.src}"]') ) {
                    ...
                    s.defer = true;
                    Object.entries(JSON.parse('${hsScripts.scriptAttributes}')).forEach(([k,v])=>s.setAttribute(k,v));
    ...
    eleventyConfig.addShortcode("hubspotForm", (formId, args = {}) => {
        ...
        if (loadingMode === "lazy") {
            ...
            const scriptAttrs = buildScriptAttrsString(hsScripts.scriptAttributes);
            const attrsStr = scriptAttrs ? ` ${scriptAttrs}` : '';
            hubspotFormCode = `<script charset="utf-8" type="text/javascript" src="${hsScripts.forms.src}"${attrsStr}></script><script type="text/javascript">/*<![CDATA[|*/hbspt.forms.create(JSON.parse(decodeURIComponent('${encodedConfig}')))/*]]>*/</script>`;
        }
